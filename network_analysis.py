...
SCORE = 'buy_am'
import sklearn
# 코사인 유사도를 보기 위하여 데이터프레임을 다르게 정의
df_temp = dataframe[dataframe.biz_unit.isin(BIZ_set)]
df_temp2 = df_temp[[ID,CL,SCORE]].groupby([ID,CL])[SCORE].agg('sum')
df_cos = df_temp2.unstack(level=1, fill_value=0)
cos_sim = sklearn.metrics.pairwise.cosine_similarity(df_cos  , Y = df_cos , dense_output = True )
df_cos_sim = pd.DataFrame(cos_sim)

df_cos_sim.columns = df_cos.index
df_cos_sim.index= df_cos.index
df_cos_sim.index.name=None
links = df_cos_sim.stack().reset_index()
links.columns = ['var1', 'var2','value']
links_filtered=links.loc[ (links['value'] >threshold) & (links['var1'] != links['var2']) ]
G=nx.from_pandas_edgelist(links_filtered, 'var1', 'var2')
nx.draw(G, with_labels=True, node_color='orange', node_size=50, edge_color='black', linewidths=1, font_size=10)
...
''' 영화 감상 리뷰
from sklea  rn.metrics.pairwise import linear_kernel
cosine_sim = linear_kernel(df_cos, df_cos)
cosine_sim

indices = pd.Series(range(len(df_cos)), index=df_cos.index).drop_duplicates()
print(indices.head())
...
'''
def NETcos(dataframe,BIZ_set,ID,CL,SCORE,threshold,title):
    df_temp = dataframe[dataframe.biz_unit.isin(BIZ_set)]
    df_temp2 = df_temp[[ID,CL,SCORE]].groupby([ID,CL])[SCORE].agg('sum')
    df_temp3 = df_temp2.unstack(level=-1, fill_value=0)
